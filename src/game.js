...
renderer.setPixelRatio(Math.min(devicePixelRatio,2));renderer.setSize(innerWidth,innerHeight);renderer.shadowMap.enabled=true;document.body.appendChild(renderer.domElement);
...
const player={pos:new THREE.Vector3(0,1.2,4),vel:new THREE.Vector3(),hp:100,stamina:100,weight:0,grounded:false,dead:false,climbing:false,carrying:null,pullTarget:null,attackCooldown:0};
const keys={},mouse={left:false,right:false};
const inventory=[null,null,null,null];let selected=0,chaos=0,weather='CLEAR',weatherTimer=8;
const blocks=[],items=[],creatures=[],corpses=[],physics=[];
...
function box(name,x,y,z,sx,sy,sz,color=0x68705f,physical=false,mass=1){
 ...
 m.name=name;m.position.set(x,y,z);m.userData.size={x:sx,y:sy,z:sz};m.userData.mass=mass;m.userData.vel=new THREE.Vector3();m.castShadow=true;m.receiveShadow=true;scene.add(m);blocks.push(m);if(physical)physics.push(m);return m;
}
...
// Ana rota: sabit omurga + küçük rastgele sapmalar.
...
function pickup(x,y,z,type='basic'){
 const color=type==='medkit'?0xe6e6e6:type==='ammo'?0x5d8bd3:0xd6a735;
 const m=new THREE.Mesh(new THREE.BoxGeometry(.45,.45,.45),new THREE.MeshStandardMaterial({color}));
 m.position.set(x,y,z);m.userData={type};scene.add(m);items.push(m);
}
// Temel ekipman %100 spawn olur.
...
function spawnFlying(kind='scout'){
 const data={scout:{r:.7,hp:3,mass:2,speed:2.2,color:0x705050},bat:{r:.5,hp:2,mass:1,speed:3.5,color:0x4b3d65},stinger:{r:.45,hp:5,mass:3,speed:1.7,color:0x80652f}}[kind];
 const c=sphere((Math.random()-.5)*12,3+Math.random()*5,-20-Math.random()*70,data.r,data.color);
 c.userData={type:'flying',subtype:kind,hp:data.hp,maxHp:data.hp,baseY:c.position.y,vel:new THREE.Vector3(),mass:data.mass,speed:data.speed,phase:Math.random()*10};creatures.push(c);
}
function spawnClimber(kind='crawler'){
 const data={crawler:{r:.65,hp:4,mass:3,speed:1.8,color:0x465f70},brute:{r:.9,hp:9,mass:9,speed:.8,color:0x5d463d}}[kind];
 const c=sphere((Math.random()>.5?1:-1)*(3+Math.random()*3),.9,-25-Math.random()*60,data.r,data.color);
 c.userData={type:'climber',subtype:kind,hp:data.hp,maxHp:data.hp,vel:new THREE.Vector3(),mass:data.mass,speed:data.speed,phase:Math.random()*10};creatures.push(c);
}
for(let i=0;i<2;i++)spawnFlying('scout');spawnFlying('bat');spawnClimber('crawler');spawnClimber('brute');
...
function updateSlots(){const r=document.querySelector('#slots');if(r)r.innerHTML=inventory.map((v,i)=>`<div class="slot ${i===selected?'selected':''}">${i+1}<br>${v||'—'}</div>`).join('');}
function useSelected(){
 if(inventory[selected]==='medkit'){player.hp=Math.min(100,player.hp+35);inventory[selected]=null;document.querySelector('#message').textContent='İlk yardım kullanıldı';}
 else if(inventory[selected]==='ammo'){document.querySelector('#message').textContent='Cephane hazır — ateş etmek için sol tık';}
 updateSlots();
}
...
function targetPhysicsObject(max=2.8){
 const ray=new THREE.Raycaster(camera.position,forwardVector(),0,max);
 const hits=ray.intersectObjects(physics,false);return hits.length?hits[0].object:null;
}
function pullObject(){
 if(player.carrying)return;
 const o=targetPhysicsObject(3.2);
 if(!o)return;
 if(player.pullTarget===o){player.pullTarget=null;return;}
 player.pullTarget=o;
}

function die(){
 if(player.dead)return;player.dead=true;player.hp=0;
 const c=box('corpse',player.pos.x,.65,player.pos.z,.8,1.3,.45,0x9b7770,true,70);c.userData.weight=1;corpses.push(c);
...
addEventListener('keydown',e=>{
 keys[e.code]=true;
 if(e.code.startsWith('Digit')){selected=Math.min(3,Number(e.code.slice(-1))-1);updateSlots();}
 if(e.code==='KeyE'){
  if(nearestItem())interact();
  else if(nearCorpse()||player.carrying)toggleCarry();
  else useSelected();
 }
...
addEventListener('mousedown',e=>{
 if(e.button===0){mouse.left=true;shootOrMelee();}
...
function shootOrMelee(){
 ...
 if(hits.length&&hits[0].distance<3.2){
  ...
  const push=ray.ray.direction.clone().multiplyScalar(8/(c.userData.mass||1));c.userData.vel.add(push);
  player.attackCooldown=.35;return;
 }
 const longHits=ray.intersectObjects(creatures,false);
 if(longHits.length&&longHits[0].distance<35){
  const c=longHits[0].object;c.userData.hp-=2;c.userData.vel.add(ray.ray.direction.clone().multiplyScalar(7/(c.userData.mass||1)));
  if(c.userData.hp<=0){scene.remove(c);creatures.splice(creatures.indexOf(c),1);}player.attackCooldown=.15;
 }
 for(const o of physics){const h=ray.intersectObject(o,false);if(h.length&&h[0].distance<15)o.userData.vel.add(ray.ray.direction.clone().multiplyScalar(10/(o.userData.mass||1)));}
}

function tryClimb(dt){
 const f=forwardVector();
 const hit=new THREE.Raycaster(camera.position,f,0,1.5).intersectObjects(blocks,false).find(h=>h.object.name==='climbWall');
...
function updatePhysics(dt){
 for(const o of physics){
  ...
  if(player.pullTarget===o){const to=player.pos.clone().sub(o.position);to.y=0;const d=to.length();if(d<1.5)player.pullTarget=null;else v.addScaledVector(to.normalize(),(7/(o.userData.mass||1))*dt);}
...
function updateCreatures(dt,now){
 for(const c of [...creatures]){
  ...
  if(u.type==='flying'){
   ...
   if(d<28){to.normalize();u.vel.lerp(to.multiplyScalar(u.speed),.035);}
   else u.vel.multiplyScalar(.96);
   u.vel.y+=Math.sin(now*.002+u.phase)*.015;c.position.addScaledVector(u.vel,dt);
   c.position.y=Math.max(1.4,c.position.y);
  }else{
   const to=player.pos.clone().sub(c.position);to.y=0;const d=to.length();
   if(d<22&&d>1.4){u.vel.lerp(to.normalize().multiplyScalar(u.speed),.04);c.position.addScaledVector(u.vel,dt);}
   else u.vel.multiplyScalar(.9);
   c.position.y=.9;
  }
  const d=c.position.distanceTo(player.pos);
  if(d<1.3&&!player.dead){
   ...
   player.hp=Math.max(0,player.hp-(u.subtype==='brute'?6:4)*dt);
  }
 }
}
...
function tick(now){
 ...
 if(!player.dead){
  ...
  if(player.pos.z<-12){
   chaos=Math.min(100,chaos+dt*2);weatherTimer-=dt;
   if(weatherTimer<=0){weatherTimer=10+Math.random()*18;weather=Math.random()<.4?'ACID RAIN':Math.random()<.35?'STORM':'CLEAR';const w=document.querySelector('#weather');if(w)w.textContent=weather;}
   if(weather==='ACID RAIN'&&Math.random()<dt*.8)player.hp=Math.max(0,player.hp-2);
   if(weather==='STORM'&&Math.random()<dt*.12)player.hp=Math.max(0,player.hp-12);
   if(Math.random()<dt*.035&&creatures.length<9){const r=Math.random();if(r<.45)spawnFlying('scout');else if(r<.7)spawnFlying('bat');else if(r<.85)spawnFlying('stinger');else spawnClimber(Math.random()<.65?'crawler':'brute');}
  }
 ...
 document.querySelector('#hp').textContent=Math.round(player.hp);document.querySelector('#stamina').textContent=Math.round(player.stamina);document.querySelector('#weight').textContent=player.weight+(player.carrying?30:0);
 document.querySelector('#chaos').textContent=chaos<25?'LOW':chaos<60?'MEDIUM':'HIGH';
 const hb=document.querySelector('#hpbar'),sb=document.querySelector('#stambar');if(hb)hb.style.width=Math.max(0,player.hp)+'%';if(sb)sb.style.width=Math.max(0,player.stamina)+'%';
